...
- **Names reveal intent.** `getGamesByLeague` not `get`, `isValidEmail` not `check`
...
| **I**diomatic | Feels natural in TypeScript/React, follows project conventions |
| **D**omain-based | Structure mirrors the application domain |
...
- **Long parameter lists** — group into objects (use destructuring)
- **Feature envy** — method uses another module's data more than its own
- **Primitive obsession** — use domain types (`MovieId`, `ReviewId`) not raw strings
